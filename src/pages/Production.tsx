...
export default function Production() {
  ...
  const getStatusInfo = () => {
    const variants = {
      idle: { label: 'Ready', description: 'Configure settings and generate', icon: Zap },
    ...
    return (
      <div className="relative flex flex-col items-center justify-center min-h-[80vh] p-6">
        <div className="absolute inset-0 overflow-hidden pointer-events-none">
          <div className="absolute top-1/4 left-1/4 w-[500px] h-[500px] bg-primary/5 rounded-full blur-[120px] orb-1" />
        </div>
        ...
        <div className="relative z-10 text-center space-y-6 animate-fade-in">
          ...
          <div className="space-y-2">
            ...
            <p className="text-muted-foreground">Select a project to begin production</p>
          </div>
  ...
  return (
    <div className="relative flex h-[calc(100vh-3.5rem)]">
      ...
      <div className="relative z-10 flex-1 flex flex-col overflow-hidden">
        {/* Header */}
        <header className="px-6 py-4 border-b border-border/10 bg-background/30 backdrop-blur-xl">
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-3">
              <Badge variant={status as 'idle' | 'generating' | 'rendering' | 'completed'} className="gap-1.5 px-3 py-1">
          ...
          <div className="max-w-5xl mx-auto space-y-6 animate-fade-in">
            ...
            <div className="video-container">
              <div className="relative aspect-video bg-gradient-to-br from-muted/10 via-background to-muted/20">
                {/* Grid */}
                <div className="absolute inset-0 bg-[linear-gradient(hsl(var(--border)/0.03)_1px,transparent_1px),linear-gradient(90deg,hsl(var(--border)/0.03)_1px,transparent_1px)] bg-[size:32px_32px]" />
                  ...
                  <div className="absolute inset-0">
                    ...
                    >
                      <div className={cn(
                        ...
                        <div className="absolute inset-0 rounded-2xl bg-primary blur-2xl opacity-40 group-hover:opacity-60 transition-opacity" />
                        {isPlaying ? (
                ...
                {(status === 'generating' || status === 'rendering') && (
                  <div className="absolute inset-0 flex items-center justify-center bg-background/80 backdrop-blur-xl">
                    <div className="text-center space-y-8">
                      ...
                      <div className="space-y-2">
                        <p className="text-xl font-display text-foreground">{statusInfo.label}</p>
                ...
                {status === 'idle' && (
                  <div className="absolute inset-0 flex items-center justify-center">
                    <div className="text-center space-y-5">
                      <div className="w-20 h-20 mx-auto rounded-2xl bg-muted/20 border border-border/20 flex items-center justify-center">
                        <Play className="w-8 h-8 text-muted-foreground" />
                      </div>
                      <div className="space-y-1">
                        <p className="text-foreground font-medium">Ready to Create</p>
                        <p className="text-muted-foreground text-sm">Click Generate to start</p>
                      </div>
                    </div>
                  </div>
                )}
                ...
                <div className="absolute top-4 left-4 z-20">
                  <Badge variant="outline" className="font-mono text-[10px]">16:9</Badge>
                </div>
                <div className="absolute top-4 right-4 z-20">
                  <Badge variant="outline" className="font-mono text-[10px]">4K UHD</Badge>
                </div>
              </div>

              {/* Controls */}
              <div className="relative px-4 py-3 border-t border-border/10 bg-card/40 backdrop-blur-sm">
                <div className="mb-3">
                  <Slider
                    value={[progress]}
                ...
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-1">
                    <Button variant="ghost" size="icon" className="h-8 w-8 control-btn" disabled={status !== 'completed'}>
                      <SkipBack className="w-4 h-4" />
                    </Button>
                    <Button variant="ghost" size="icon" className="h-9 w-9 control-btn" disabled={status !== 'completed'} onClick={() => setIsPlaying(!isPlaying)}>
                    ...
                    <div className="flex items-center gap-2 ml-2 pl-2 border-l border-border/20">
                      <Button variant="ghost" size="icon" className="h-8 w-8 control-btn" disabled={status !== 'completed'} onClick={() => setIsMuted(!isMuted)}>
                        {isMuted ? <VolumeX className="w-4 h-4" /> : <Volume2 className="w-4 h-4" />}
                      </Button>
                      <Slider value={[isMuted ? 0 : volume]} onValueChange={(val) => setVolume(val[0])} max={100} className="w-20" disabled={status !== 'completed'} />
                    </div>
                  </div>
      ...
      {/* Settings Sidebar */}
      <aside className="hidden xl:flex w-80 border-l border-border/10 bg-sidebar/30 backdrop-blur-xl overflow-y-auto">
