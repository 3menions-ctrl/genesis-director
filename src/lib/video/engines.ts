...
export function renderSurchargeCredits(
  ...
  const blocks = Math.max(1, Math.ceil(totalDurationSec / 10)); // per started 10s
  let c = 0;
  if (opts.upscale4k) c += blocks * spec.upscale4kCredits;
  if (opts.fps60) c += blocks * spec.fps60Credits;
  return c;
}
