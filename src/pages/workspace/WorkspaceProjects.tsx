  ...
  return (
    ...
    >
      <Surface padded={false}>
        <div className="flex items-center justify-between px-5 py-3 border-b border-[hsl(35,12%,12%)]">
        ...
        {!loading && rows.length === 0 ? (
          ...
          <ul className="divide-y divide-[hsl(35,12%,12%)]">
            {rows.map((p) => (
              <li key={p.id} className="px-5 py-3 flex items-center gap-4 hover:bg-[hsl(35,12%,6%)] transition-colors">
                <div className="w-12 h-12 bg-[hsl(35,12%,8%)] border border-[hsl(35,12%,14%)] overflow-hidden shrink-0">
                  {p.thumbnail_url
                    ? <img src={p.thumbnail_url} alt="" className="w-full h-full object-cover" />
                    : <div className="w-full h-full flex items-center justify-center"><Film className="w-3.5 h-3.5 text-[hsl(35,8%,40%)]" /></div>}
                </div>
                <div className="min-w-0 flex-1">
                  <div className="text-[13px] text-[hsl(35,12%,92%)] truncate">{p.title || 'Untitled project'}</div>
                  <div className="font-mono text-[10px] uppercase tracking-[0.18em] text-[hsl(35,8%,50%)] mt-1">
