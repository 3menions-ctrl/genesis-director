...
export const ColorGradingPanel = ({ clip, onUpdateClip }: ColorGradingPanelProps) => {
  ...
  const update = (key: keyof ColorGrading, value: number) => {
    onUpdateClip(clip.id, { colorGrading: { ...grading, [key]: value } });
  };
  ...
  return (
    <div className="space-y-3">
      <div className="flex items-center justify-between">
        <span className="text-[9px] font-semibold uppercase tracking-[0.2em] text-white/30">Color Grading</span>
        <Button
          variant="ghost" size="icon"
          className={`h-5 w-5 rounded transition-all ${isModified ? 'text-white hover:text-white hover:bg-white/[0.1]' : 'text-white/15'}`}
          ...
          <div key={key} className="space-y-1">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-1.5">
                <Icon className={`h-3 w-3 ${isChanged ? 'text-white' : 'text-white/20'} transition-colors`} />
            ...
            <Slider value={[val]} min={min} max={max} step={1} onValueChange={([v]) => update(key, v)} className="h-4" />
          </div>
        );
      })}
