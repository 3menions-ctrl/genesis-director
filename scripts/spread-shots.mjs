...
const ID=process.env.BIZ_ID||"bb77364d-047b-4abb-a726-eca49d33e40d",EM=process.env.BIZ_EMAIL||"demo-mira@smallbridges.test",P=`Sp!${Date.now()}x`;
...
for(const [path,name] of shots){
  try{await pg.goto(`http://localhost:7788${path}`,{waitUntil:"networkidle",timeout:60000});await pg.waitForTimeout(2800);
  await pg.screenshot({path:`/tmp/${name}.png`,fullPage:true});console.log("ok",name,pg.url());}
