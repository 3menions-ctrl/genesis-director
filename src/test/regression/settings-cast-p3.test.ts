...
const cast = readFileSync(resolve(REPO_ROOT, "src/pages/Cast.tsx"), "utf-8");
const dash = readFileSync(
  resolve(REPO_ROOT, "src/pages/account/SettingsDashboard.tsx"),
  "utf-8",
);

describe("Cast delete confirms first", () => {
  it("remove() gates on confirmAsync before deleting", () => {
    const region = cast.slice(cast.indexOf("const remove ="), cast.indexOf("const remove =") + 500);
    expect(region).toMatch(/confirmAsync\(/);
    expect(region).toMatch(/if\s*\(!ok\)\s*return/);
  });
});
