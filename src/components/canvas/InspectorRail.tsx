    ...
    <aside className="w-[360px] shrink-0 border-l border-white/[0.06] bg-[hsl(220,14%,2.5%)]/90 backdrop-blur-2xl flex flex-col">
      <div className="px-6 py-4 border-b border-white/[0.05] flex items-center justify-between">
        <div className="flex items-center gap-2 text-[10px] uppercase tracking-[0.28em] text-white/45">
          <Sparkles className="h-3 w-3 text-[#0A84FF]" /> Inspector
        </div>
        {node && (
      ...
      <Section title="Audio" sub="Score · voice · FX">
        <Field label="Source">
          <select value={(d as any).source}
            ...
            className="w-full rounded-md bg-white/[0.04] border border-white/10 px-3 py-2 text-sm">
    ...
    return (
      <Section title="Render" sub="Bake the cut">
        <p className="text-xs text-white/55 leading-relaxed">
          ...
          <span className="text-[#9CC2FF]"> Render </span> in the toolbar to dispatch.
        </p>
      </Section>
    );
  }
            ...
            <>
              ...
              {chips.length > 0 && (
                <div className="flex flex-wrap gap-1.5 mt-3">
                  {chips.map((c) => (
                    <span key={c} className="text-[9.5px] uppercase tracking-[0.18em] px-1.5 py-0.5 rounded-md bg-white/[0.06] text-white/70 border border-white/10">
                      {c}
                    </span>
                  ))}
                </div>
          ...
          <div className="space-y-1">
            ...
            {results.map((m) => (
              <button key={`${m.owner}/${m.name}`} onClick={() => choose(m)}
                className="w-full text-left px-3 py-2 rounded-lg hover:bg-white/[0.04] border border-white/[0.05] hover:border-white/15">
            ...
            return (
              <button key={`${m.owner}/${m.name}`} onClick={() => choose(m)}
                className={`w-full text-left px-3 py-2 rounded-lg border transition-all ${
                  active ? 'border-[#0A84FF]/60 bg-[#0A84FF]/[0.06]' : 'border-white/[0.05] hover:border-white/15 hover:bg-white/[0.03]'
                }`}>
                  ...
                  return (
                    <button key={a} onClick={() => setInputs({ aspectRatio: a })}
                      className={`text-[11px] px-2.5 py-1 rounded-md border transition ${
                        on ? 'border-[#0A84FF]/60 bg-[#0A84FF]/15 text-white' : 'border-white/10 text-white/60 hover:border-white/25'
                      }`}>
                      {a}
                    </button>
                  );
        ...
        <div className="grid grid-cols-3 gap-2">
          {avatars.map((a) => (
            <button key={a.id} onClick={() => update({ avatarId: a.id, imageUrl: a.image_url, name: a.name, label: a.name })}
              className={`rounded-xl overflow-hidden border ${data.avatarId === a.id ? 'border-[#0A84FF]' : 'border-white/10'} hover:border-white/30 transition`}>
