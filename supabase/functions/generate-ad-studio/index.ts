...
Generate ${conceptCount} concept${conceptCount > 1 ? "s" : ""} now.`;
    ...
    // run this OpenAI ad generation unlimited for free. deduct_credits is the
    // canonical, org-aware spend RPC (returns false on insufficient balance).
    // Service-role/internal callers are exempt. NOTE: charges the caller's
    // personal ledger; org-pool routing for business use is a follow-up.
    if (!auth.isServiceRole && auth.userId) {
      ...
      const { data: charged, error: chargeErr } = await billing.rpc("deduct_credits", {
        ...
        p_amount: 3,
        p_description: "Ad Studio generation",
      });
