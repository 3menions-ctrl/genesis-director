  ...
  return (
    ...
    >
      <Surface padded={false}>
          ...
          <table className="w-full text-left">
            <thead>
              <tr className="border-b border-[hsl(35,12%,12%)]">
                <th className="px-5 py-3 font-mono text-[10px] uppercase tracking-[0.24em] text-[hsl(35,8%,55%)]">Capability</th>
                {ROLES.map(r => (
                  <th key={r} className="px-3 py-3 font-mono text-[10px] uppercase tracking-[0.24em] text-[hsl(35,8%,55%)] text-center">{r}</th>
                ))}
              </tr>
            </thead>
            <tbody>
              {MATRIX.map(row => (
                <tr key={row.capability} className="border-b border-[hsl(35,12%,10%)]">
                  <td className="px-5 py-3 text-[12px] text-[hsl(35,12%,90%)]">{row.capability}</td>
                  {ROLES.map(r => (
                    <td key={r} className="px-3 py-3 text-center">
                      {row.allowed.includes(r)
                        ? <span className="inline-block w-1.5 h-1.5 rounded-full bg-[hsl(28,90%,60%)]" />
                        : <span className="inline-block w-1.5 h-1.5 rounded-full bg-[hsl(35,12%,14%)]" />}
                    </td>
                  ))}
                </tr>
              ))}
        ...
        <div className="px-5 py-3 border-t border-[hsl(35,12%,12%)] flex items-center gap-2">
          <Pill tone="amber">READ-ONLY</Pill>
          <span className="text-[11px] text-[hsl(35,8%,55%)] font-mono">Custom role policies coming with Enterprise tier.</span>
        </div>
      </Surface>
    </WorkspacePage>
  );
