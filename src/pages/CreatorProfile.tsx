      ...
      {!isLoading && profile && id && (
        <div className="fixed z-30 flex items-center gap-2" style={{ top: 'calc(var(--safe-top,0px) + 12px)', right: '14px' }}>
          <button onClick={async () => { const r = await shareLink({ title: `${profile.display_name ?? 'A creator'} on Small Bridges`, text: `Watch ${profile.display_name ?? 'their'} films`, url: `https://smallbridges.co/c/${id}` }); if (r === 'copied') toast.success('Link copied'); }}
            aria-label="Share profile" className="grid h-10 w-10 place-items-center rounded-full bg-white/[0.06] ring-1 ring-white/25 backdrop-blur-md text-white drop-shadow-[0_2px_10px_rgba(0,0,0,.55)] transition-transform active:scale-90"><Share2 className="h-[18px] w-[18px]" /></button>
          {!isSelf && (
            <div className="relative">
              <button onClick={() => { void hapticTap(); setMenuOpen((o) => !o); }} aria-label="More" className="grid h-10 w-10 place-items-center rounded-full bg-white/[0.06] ring-1 ring-white/25 backdrop-blur-md text-white drop-shadow-[0_2px_10px_rgba(0,0,0,.55)] transition-transform active:scale-90"><MoreVertical className="h-[18px] w-[18px]" /></button>
              {menuOpen && (
                <>
