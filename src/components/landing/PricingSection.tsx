...
const ImmersiveVideoBackground = memo(function ImmersiveVideoBackground({ onClose }: { onClose: () => void }) {
  ...
  return (
    <div className="fixed inset-0 z-[45] animate-fade-in" style={{ animationDuration: '1.2s' }}>
      ...
      <div className="absolute inset-0 bg-gradient-to-b from-black/60 via-black/40 to-black/65 pointer-events-none" />
      
      {/* Floating controls - top right */}
      <div className="fixed top-20 right-6 z-[60] flex items-center gap-3 animate-fade-in" style={{ animationDelay: '0.6s' }}>
        {/* Label */}
        ...
        <button
          onClick={onClose}
          className="group flex items-center justify-center w-10 h-10 rounded-full bg-black/50 backdrop-blur-xl border border-white/10 hover:bg-white/10 hover:border-white/20 transition-all duration-300"
