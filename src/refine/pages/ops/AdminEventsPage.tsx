...
export default function AdminEventsPage() {
  ...
  const columns = useMemo(() => [
    col.accessor("event", { header: "Event", cell: (c) => <span className="font-mono text-[13px] text-[#0c1426]">{c.getValue()}</span> }),
    ...
    col.accessor("actors", { header: "Actors", cell: (c) => <span className="tabular-nums text-[#5d6a82]">{Number(c.getValue()).toLocaleString()}</span> }),
  ], []);
  ...
  return (
    <div className="mx-auto w-full max-w-7xl px-6 py-8 sm:px-8">
      <AdminPageHeader
        ...
        actions={
          <button onClick={() => setLive((v) => !v)} className="inline-flex items-center gap-2 rounded-full bg-[#f6f8fc] px-4 py-2 font-mono text-[10px] uppercase tracking-[0.22em] text-[#5d6a82] transition-colors hover:bg-[#f4f7ff] hover:text-[#0c1426]">
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-[1.2fr_1fr]">
        <div>
          <div className="mb-4 font-mono text-[10px] uppercase tracking-[0.24em] text-[#9aa4b8]">Events · last 7 days</div>
          {loading ? <div className="py-16 text-center font-mono text-[11px] uppercase tracking-[0.22em] text-[#9aa4b8]">Loading…</div>
            : <DataTable columns={columns as never} data={counts} dense empty="No events tracked yet — browse the app to generate some." />}
        </div>
        <div>
          <div className="mb-4 font-mono text-[10px] uppercase tracking-[0.24em] text-[#9aa4b8]">Live feed</div>
          <AdminCard className="max-h-[520px] overflow-y-auto p-2">
            {feed.length === 0 && <div className="px-3 py-10 text-center text-[13px] font-light text-[#9aa4b8]">Waiting for events…</div>}
            {feed.map((e) => (
              <div key={e.id} className="flex items-center gap-3 rounded-xl px-3 py-2.5 transition-colors hover:bg-[#f4f7ff]">
                <span className="h-1.5 w-1.5 shrink-0 rounded-full" style={{ background: ACCENT_HSL, boxShadow: `0 0 8px ${accent(0.8)}` }} />
                <span className="font-mono text-[12px] text-[#0c1426]">{e.name}</span>
                <span className="truncate font-mono text-[11px] text-[#9aa4b8]">{e.path}</span>
                <span className="ml-auto shrink-0 font-mono text-[10px] text-[#9aa4b8]">{actor(e)}</span>
                <span className="shrink-0 font-mono text-[10px] text-[#9aa4b8]">{ago(e.occurred_at)}</span>
              </div>
            ))}
          </AdminCard>
        </div>
