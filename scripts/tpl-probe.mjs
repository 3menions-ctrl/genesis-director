...
const stamp=Date.now(), email=`prb-${stamp}@smallbridges.test`, pass=`Ux!${stamp}aB`;
...
await fetch(`${URL}/rest/v1/rpc/add_credits`,{method:'POST',headers:{apikey:SR,Authorization:`Bearer ${SR}`,'Content-Type':'application/json'},body:JSON.stringify({p_user_id:uid,p_amount:4000,p_description:'p',p_stripe_payment_id:`P_${stamp}`})});
...
let allToasts=[];
page.on('console',m=>{const t=m.text(); if(/loaded|template|breakout/i.test(t)) console.log('  C>',t.slice(0,120));});
await page.goto('http://localhost:7778/studio?template=template-noir-1',{waitUntil:'domcontentloaded',timeout:60000});
for(const w of [2000,3000,3000]){
  await page.waitForTimeout(w);
  const d=await page.evaluate(()=>({
    tas:[...document.querySelectorAll('textarea')].map(t=>(t.value||'').length),
    toasts:[...document.querySelectorAll('[data-sonner-toast]')].map(t=>(t.innerText||'').slice(0,60)),
    tabs:[...document.querySelectorAll('[role=tab],[aria-selected]')].map(t=>(t.getAttribute('aria-selected')+':'+(t.innerText||'').slice(0,12))),
  }));
  console.log('textareaLens=',JSON.stringify(d.tas),'toasts=',JSON.stringify(d.toasts));
}
await b.close();
