  ...
  return (
    <div className="fixed inset-0 overflow-y-auto text-white">
      ...
      <div className="fixed inset-x-0 z-20 flex flex-col items-center" style={{ bottom: 'calc(var(--safe-bottom,0px) + var(--tabbar-h,0px) + 16px)' }}>
        <button onClick={goLive} disabled={going} aria-label="Go live" className="grid h-[68px] w-[68px] place-items-center rounded-full bg-gradient-to-br from-[#ff3b5c] to-[#ff8a3b] text-white shadow-[inset_0_1px_0_rgba(255,255,255,.3),0_20px_44px_-14px_rgba(255,59,92,.7)] transition-transform active:scale-90 disabled:opacity-50">
  ...
  return (
    <div className="fixed inset-0 overflow-hidden bg-black text-white">
      ...
      <div className="absolute inset-x-0 z-30 flex items-center gap-2 px-3" style={{ bottom: 'calc(var(--safe-bottom,0px) + 14px)' }}>
        ...
        {!isHost && (
          <button onClick={() => { void hapticTap(); host?.topReel ? setGiftOpen(true) : react('🎉'); }} aria-label="Gift" className="grid h-11 w-11 place-items-center rounded-full bg-gradient-to-br from-[#2f6bff] to-[#7a3bff] active:scale-90"><GiftIcon className="h-[20px] w-[20px]" /></button>
        )}
      </div>
