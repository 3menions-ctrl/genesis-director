...
CREATE OR REPLACE FUNCTION public.admin_list_projects(p_limit integer DEFAULT 100, p_offset integer DEFAULT 0, p_status text DEFAULT NULL::text, p_search text DEFAULT NULL::text, p_sort_by text DEFAULT 'updated_at'::text, p_sort_order text DEFAULT 'desc'::text)
 RETURNS TABLE(id uuid, user_id uuid, title text, status text, genre text, mode text, is_public boolean, video_url text, thumbnail_url text, created_at timestamp with time zone, updated_at timestamp with time zone, likes_count integer, aspect_ratio text, user_email text, user_name text, clips_total bigint, clips_completed bigint, clips_failed bigint, clips_pending bigint)
 LANGUAGE plpgsql
 SECURITY DEFINER
