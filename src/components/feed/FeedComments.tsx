...
export function FeedComments({ open, reelId, isStatic, onClose, onPosted }: Props) {
  ...
  return (
    <div className={cn('fixed inset-0 z-[60]', open ? 'pointer-events-auto' : 'pointer-events-none')}>
      ...
      <div
        className="absolute inset-x-0 bottom-0 flex max-h-[78%] flex-col rounded-t-[28px] bg-[#0d0d14]/85 backdrop-blur-2xl shadow-[0_-24px_70px_-24px_rgba(0,0,0,.9),inset_0_1px_0_rgba(255,255,255,.08)] transition-transform duration-300"
        style={{ transform: open ? 'translateY(0)' : 'translateY(110%)', paddingBottom: 'calc(var(--safe-bottom,0px) + var(--tabbar-h,0px) + 12px)' }}
      >
