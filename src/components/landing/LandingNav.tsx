...
export const LandingNav = memo(forwardRef<HTMLElement, NavigationProps>(
  function LandingNav({ onScrollToSection, onNavigate, onGetStarted, signUpButtonRef }, ref) {
    return (
      ...
      >
        {/* Frosted glass bar */}
        <div className="max-w-7xl mx-auto mt-4 flex items-center justify-between h-14 px-6 rounded-2xl bg-glass-hover backdrop-blur-2xl border border-white/[0.06] shadow-[0_2px_24px_rgba(0,0,0,0.25)]">
          ...
          <div className="hidden md:flex items-center gap-1">
            {SCROLL_ITEMS.map((item) => (
              <button
                ...
                className="px-4 py-1.5 text-[13px] text-white/45 hover:text-white rounded-lg hover:bg-glass-active transition-all duration-200 font-medium tracking-wide"
              >
                {item}
              </button>
            ))}
          ...
          <div className="flex items-center gap-2">
            <Button
              ...
              className="h-8 px-4 text-[13px] text-white/50 hover:text-white hover:bg-glass-active rounded-lg font-medium"
            >
              Sign in
            </Button>
            <Button
