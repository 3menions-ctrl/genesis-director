/**
 * EditorStudioAurora — fixed cinematic backdrop for the Editor page.
 * Mirrors the Create page's StudioAurora so the Editor inherits its
 * "premium / floating words / boundary-less" feel:
 *   1. Deep radial base
 *   2. Slow conic aurora sweep
 *   3. Primary blue aurora + cool counter-glow
 *   4. Edge vignette + film grain
 *   5. Top hairline highlight
 */
import { memo } from 'react';

export const EditorStudioAurora = memo(function EditorStudioAurora() {
  return (
    <div aria-hidden className="pointer-events-none fixed inset-0 -z-10 overflow-hidden">
      <style>{`
        @keyframes editorAurora { 0%{transform:rotate(0deg)} 100%{transform:rotate(360deg)} }
      `}</style>
      ...
      <div
        ...
        style={{
          ...
          filter: 'blur(80px)',
          animation: 'editorAurora 60s linear infinite',
        }}
      />

      {/* primary aurora */}
      <div
        className="absolute -top-1/3 left-1/2 h-[60vmax] w-[60vmax] -translate-x-1/2 rounded-full opacity-[0.32] blur-3xl animate-[pulse_12s_ease-in-out_infinite]"
...
export default EditorStudioAurora;