...
export default function AdminStorageBillingPage() {
  ...
  const cols = useMemo(() => [
    ...
    col.accessor("objects", { header: "Objects", cell: (c) => <span className="tabular-nums text-[#5d6a82]">{Number(c.getValue()).toLocaleString()}</span> }),
    col.accessor("billable_gb", { header: "Billable", cell: (c) => <span className="tabular-nums text-[#5d6a82]">{Number(c.getValue()).toFixed(3)} GB</span> }),
    col.accessor("est_charge_credits", { header: "Est. charge", cell: (c) => <span className="tabular-nums text-[#0c1426]">{Number(c.getValue()).toLocaleString()} cr</span> }),
  ], []);
  ...
  return (
    <div className="mx-auto w-full max-w-7xl px-6 py-8 sm:px-8">
      <AdminPageHeader eyebrow="Money" title={<>Storage <span className="italic">billing</span>.</>} sub="Per-user storage metered and billed in credits above the free tier — priced for margin, posted to the ledger."
        actions={
          <div className="flex items-center gap-2">
            <button onClick={recompute} disabled={!!busy} className="inline-flex items-center gap-2 rounded-full bg-[#f6f8fc] px-4 py-2 font-mono text-[10px] uppercase tracking-[0.2em] text-[#5d6a82] hover:bg-[#f4f7ff] hover:text-[#0c1426] disabled:opacity-40"><RefreshCw className={busy === "compute" ? "h-3 w-3 animate-spin" : "h-3 w-3"} /> Recompute</button>
      ...
      <AdminCard className="mb-6 p-5">
        <div className="flex flex-wrap items-center gap-x-8 gap-y-2 font-mono text-[11px] text-[#5d6a82]">
          ...
          <span className="text-[#9aa4b8]">Snapshot: {d?.snapshot_day ?? "—"}</span>
        </div>
      </AdminCard>

      <div className="mb-4 font-mono text-[10px] uppercase tracking-[0.24em] text-[#9aa4b8]">Per-user storage</div>
