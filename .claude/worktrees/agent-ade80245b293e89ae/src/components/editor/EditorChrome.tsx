...
export function EditorChrome({
  ...
  const saveProject = useCallback(async (fileName?: string) => {
    ...
    try {
      ...
      surfaceError(err, "Couldn't save — check your connection and try again.", {
        surface: 'editor.chrome',
        action: 'save-project',
        extra: { sessionId: sessionId ?? null, hasTitle: !!sessionTitle },
      });
      return { status: false, message: err?.message ?? 'Save failed' };
    } finally {
      setSaving(false);
    }
