...
const stamp=Date.now(), email=`tplchk-${stamp}@smallbridges.test`, pass=`Ux!${stamp}aB`;
...
await fetch(`${URL}/rest/v1/rpc/add_credits`,{method:'POST',headers:{apikey:SR,Authorization:`Bearer ${SR}`,'Content-Type':'application/json'},body:JSON.stringify({p_user_id:uid,p_amount:4000,p_description:'chk',p_stripe_payment_id:`CHK_${stamp}`})});
...
for(const id of ['template-noir-1','post-escape']){
  const ctx=await browser.newContext({viewport:{width:1440,height:900}});
  await ctx.addInitScript(([k,v])=>localStorage.setItem(k,v),[KEY,JSON.stringify(session)]);
  const page=await ctx.newPage();
  let tplToast='';
  page.on('console',m=>{const t=m.text(); if(/template.*loaded|breakout mode/i.test(t)) tplToast+=t.slice(0,80)+';';});
  ...
  const toastTxt=await page.evaluate(()=>{const o=[];document.querySelectorAll('[data-sonner-toast]').forEach(t=>o.push((t.innerText||'').slice(0,60)));return o;});
  console.log(`${id}: promptLen=${promptLen} consoleTpl="${tplToast}" toasts=${JSON.stringify(toastTxt)}`);
  await ctx.close();
}
await browser.close();
