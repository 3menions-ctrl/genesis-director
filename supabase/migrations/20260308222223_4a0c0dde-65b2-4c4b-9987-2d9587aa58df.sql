DROP FUNCTION IF EXISTS public.admin_list_projects(integer, integer, text, text, text, text);
...
RETURNS TABLE(
  id uuid, user_id uuid, title text, status text, genre text, mode text,
  is_public boolean, video_url text, thumbnail_url text,
  created_at timestamp with time zone, updated_at timestamp with time zone,
  likes_count integer, aspect_ratio text,
  user_email text, user_name text,
  clips_total bigint, clips_completed bigint, clips_failed bigint, clips_pending bigint,
  pipeline_stage text, quality_tier text, target_duration_minutes integer,
  last_error text, stitch_attempts integer,
  hls_playlist_url text, pending_video_tasks jsonb
)
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  SELECT
    ...
    mp.created_at,
    mp.updated_at,
    mp.likes_count::integer,
    mp.aspect_ratio::text,
    ...
    mp.pipeline_stage::text,
    mp.quality_tier::text,
    COALESCE(mp.target_duration_minutes, 1)::integer,
    mp.last_error::text,
    COALESCE(mp.stitch_attempts, 0)::integer,
    (mp.pending_video_tasks->>'hlsPlaylistUrl')::text AS hls_playlist_url,
    mp.pending_video_tasks
