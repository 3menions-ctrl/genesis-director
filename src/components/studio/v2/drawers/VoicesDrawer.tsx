...
export function VoicesDrawerContent({ selectedId, onSelect }: Props) {
  ...
  if (loading) return <div className="p-7 space-y-2">{Array.from({length:6}).map((_,i)=>(<div key={i} className="h-16 rounded-xl bg-glass-hover animate-pulse" />))}</div>;
      ...
      {voices.map(v => (
        <div key={v.voice_id}
          className={cn("rounded-xl border p-4 flex items-center gap-3 transition-all",
            selectedId === v.voice_id ? "border-primary bg-primary/[0.06]" : "border-white/[0.06] bg-glass hover:border-white/20")}>
          <button onClick={() => play(v.sample_audio_url || undefined, v.voice_id)}
            className="w-10 h-10 rounded-full bg-glass-active hover:bg-primary/20 flex items-center justify-center text-white/70">
            <Play className={cn("w-4 h-4", playing === v.voice_id && "animate-pulse text-primary")} />
          </button>
          ...
          {selectedId === v.voice_id && <Check className="w-4 h-4 text-primary" />}
        </div>
      ))}
    </div>
  );
