  ...
  return (
    <div className="mx-auto w-full max-w-7xl px-6 py-8 sm:px-8">
      ...
      <ChartCard title="Lifecycle funnel" meta="signup → paid · backfilled" className="mb-6">
        {loading ? <div className="py-10 text-center font-mono text-[11px] uppercase tracking-[0.22em] text-white/40">Loading…</div> : <FunnelBars rows={lifecycle} />}
      </ChartCard>
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-2">
        ...
        <AdminCard className="p-5">
          <div className="mb-4 flex items-center gap-2.5">
            ...
            <span className="font-display text-[16px] font-semibold tracking-tight text-white">Funnel builder</span>
          </div>
          ...
          <div className="mb-3 flex flex-wrap items-center gap-2">
            ...
            {steps.map((s, i) => (
              <span key={`${s}-${i}`} className="inline-flex items-center gap-1.5 rounded-full px-2.5 py-1 font-mono text-[11px]" style={{ background: accent(0.14), color: ACCENT_HSL }}>
                <span className="text-white/40">{i + 1}</span>{s}
          ...
          <div className="mb-5 flex flex-wrap gap-1.5">
            {unusedHint && <span className="text-[12px] font-light text-white/40">No tracked events yet — browse the app to generate some.</span>}
            {available.map((e) => (
        ...
        <AdminCard className="p-5">
          <div className="mb-4 flex items-center gap-2.5">
            ...
            <span className="font-display text-[16px] font-semibold tracking-tight text-white">Top journeys</span>
            <span className="ml-auto font-mono text-[10px] uppercase tracking-[0.18em] text-white/40">page → page</span>
          </div>
          {loading ? <div className="py-10 text-center font-mono text-[11px] uppercase tracking-[0.22em] text-white/40">Loading…</div>
            : paths.length === 0 ? <div className="py-10 text-center text-[13px] font-light text-white/40">Not enough navigation data yet.</div>
            : (
                  ...
                  return (
                    <div key={i} className="relative overflow-hidden rounded-lg px-3 py-2">
                      ...
                      <div className="relative flex items-center gap-2 font-mono text-[12px]">
                        <span className="truncate text-white/70">{p.from_path || "/"}</span>
                        <ArrowRight className="h-3 w-3 shrink-0" style={{ color: ACCENT_HSL }} />
                        <span className="truncate text-white/70">{p.to_path || "/"}</span>
                        <span className="ml-auto shrink-0 tabular-nums text-white/45">{p.transitions}</span>
                      </div>
                    </div>
                  );
                })}
