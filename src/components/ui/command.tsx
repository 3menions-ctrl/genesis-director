...
>(({ className, ...props }, ref) => (
  <CommandPrimitive
    ref={ref}
    className={cn(
      "flex h-full w-full flex-col overflow-hidden rounded-xl bg-black/95 text-white backdrop-blur-2xl",
      className,
    )}
    {...props}
  />
...
>(({ className, ...props }, ref) => (
  <div className="flex items-center border-b border-white/10 px-3" cmdk-input-wrapper="">
    ...
    <CommandPrimitive.Input
      ref={ref}
      className={cn(
        "flex h-11 w-full rounded-lg bg-transparent py-3 text-sm text-white outline-none placeholder:text-white/40 disabled:cursor-not-allowed disabled:opacity-50",
        className,
      )}
      {...props}
    />
...
>(({ className, ...props }, ref) => (
  <CommandPrimitive.Item
    ref={ref}
    className={cn(
      "relative flex cursor-default select-none items-center rounded-lg px-2 py-1.5 text-sm text-white/80 outline-none transition-colors duration-200 data-[disabled=true]:pointer-events-none data-[selected='true']:bg-white/10 data-[selected=true]:text-white data-[disabled=true]:opacity-50",
      className,
    )}
    {...props}
  />
